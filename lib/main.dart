...
// }
          ...
          child: AlertDialog(
            ...
            actions: <Widget>[
              ...
              new FlatButton(
                ...
                onPressed: () {
                  if (espaciosSuficientesParaProceso(
                          marcosPorProceso(sizeNewProceso)) !=
                      -1)
                    addProceso(
                        new procesos(nombreNewProceso, 1, sizeNewProceso));
                  else
                    espera
                        .add(new procesos(nombreNewProceso, 1, sizeNewProceso));
                  setState(() {});
                  Navigator.of(context).pop();
                },
              ),
      ...
      for (int k = 0; k < marcosaUtilizar; k++) {
        ...
        for (int i = 0; i < registro[paguinaAUsar].length; i++) {
          if (registro[paguinaAUsar][i] == null) {
            ...
            registro[paguinaAUsar][i] =
                new procesos(pro.nombre, pro.color, pro.size);
  ...
  delProceso(String nombre) {
    for (int i = 0; i < registro.length; i++) {
      for (int k = 0; k < registro[i].length; k++) {
        if (registro[i][k] != null) if (registro[i][k].nombre == nombre) {
  ...
  updateProcesos() {
    for (int i = 0; i < espera.length; i++) {
      if (espera[i] != null) if (addProceso(espera[i])) {
        espera[i] = null;
      }
    }
  }
  ...
  List<Widget> procesosForma1() {
    ...
    for (int i = 0; i < registro.length; i++) {
      for (int k = 0; k < registro[i].length; k++) {
        if (registro[i][k] != null) {
          procesosList.add(ListTile(
            title: Text("Paguina " +
                (i + 1).toString() +
                "  " +
                registro[i][k].nombre +
                " Size   " +
                registro[i][k].sizeOcupado.toString()),
          ));
        } else {
          procesosList.add(ListTile(
            title:
                Text("Paguina " + (i + 1).toString() + "  " + "No hay proceso"),
          ));
        }
      }
      procesosList.add(Divider(
        color: Colors.black,
        height: 0.0,
      ));
    }
